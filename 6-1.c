...
void main()
{
    ...
    char ch;
    int k,a,b;
    struct linked_list L; //declaring head of node
    L.head=NULL;
    scanf("%c",&ch);
    while(ch!='e')
    {
...
void list_delete(struct linked_list *L,struct node* x)
{
	struct node* temp;
	temp=L->head;
    if(temp==x) //if element is first node
    {
        L->head=x->next;
    }
    else
    {
        while(temp->next->key!=x->key) //finding prev node
        {
            temp=temp->next;
        }
        temp->next=x->next; //prev node is connected to next node
    }
}
void list_delete_first(struct linked_list *L)
{
...
void list_delete_last(struct linked_list *L)
{
    ...
    else
    {
	   struct node* temp;
	   temp=L->head;
	   while(temp->next->next!=NULL) //temp - 2nd last node
	   {
		  temp=temp->next;
	   }
	   temp->next=NULL;
